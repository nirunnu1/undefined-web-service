...
const getHttp = async (path, headers = {}, params = {}) => {
  ...
  return _axios.get(path).then(res => {
    if (res.status === 200) {
...
const postHttp = async (path, data, headers = {}, params = {}) => {
  ...
  return _axios.post(path, data).then(res => {
    if (res.status === 200) {
      return {
        status: true,
        data: res.data
...
class Service {
  ...
  FormatThaiNationalID(value) {
    ...
    if (Identity < 11) {
      return `${Identity.slice(0, 1)}-${Identity.slice(1, 5)}-${Identity.slice(5, 10)}`;
    }
    if (IdentityLength < 13) {
      return `${Identity.slice(0, 1)}-${Identity.slice(1, 5)}-${Identity.slice(5, 10)}-${Identity.slice(10, 12)}`;
    }
    return `${Identity.slice(0, 1)}-${Identity.slice(1, 5)}-${Identity.slice(5, 10)}-${Identity.slice(10, 12)}-${Identity.slice(12, 13)}`;
  }
